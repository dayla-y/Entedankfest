...
import { BATTLE_CHARACTHER_ASSET_KEYS, UI_ASSET_KEYS } from "../../../assets/asset-keys.js";
import { DIRECTION } from "../../../common/direction.js";
import { exhaustiveGuard } from "../../../utils/guard.js";
import { BATTLE_MENU_OPTIONS, ATTACK_MOVE_OPTIONS, ACTIVE_BATTLE_MENU } from "./battle-menu-options.js";
import { BATTLE_UI_TEXT_STYLE } from "./battle-menu-config.js";
import { BattleIngredients } from '../../ingredients/battle-ingredients.js';
...
export class BattleMenu {
  ...
  #fleeAttempt;
  /** @type {boolean} */
  #switchIngredientAttempt;

  /**
   *
   * @param {Phaser.Scene} scene the Phaser 3 Scene the battle menu will be added to
   * @param {BattleIngredients} activePlayerIngredient the players current active Ingredient in the current battle
   * @param {boolean} [skipBattleAnimations=false] used to skip all animations tied to the battle
   */
  constructor(scene, activePlayerIngredient, skipBattleAnimations = false) {
    this.#scene = scene;
    this.#activePlayerIngredient = activePlayerIngredient;
    ...
    this.#switchIngredientAttempt = false;
    this.#createMainInfoPane();
    this.#createMainBattleMenu();
    this.#createIngredientAttackSubMenu();
  ...
  /** @type {boolean} */
  get isAttemptingToSwitchIngredients() {
    return this.#switchIngredientAttempt;
  }

  /**
   * Trigger to update the attack names after a Ingredient has changed in the battle scene.
   * @returns {void}
   */
  updateIngredientAttackSubMenu() {
    ...
    this.#activePlayerIngredient.attacks.forEach((attack, index) => {
      /** @type {Phaser.GameObjects.Text} */
  ...
  showIngredientAttackSubMenu() {
    this.#activeBattleMenu = ACTIVE_BATTLE_MENU.BATTLE_MOVE_SELECT;
    this.#moveSelectionSubBattleMenuPhaserContainerGameObject.setAlpha(1);
  }

  hideIngredientAttackSubMenu() {
    this.#activeBattleMenu = ACTIVE_BATTLE_MENU.BATTLE_MAIN;
  ...
  #createMainBattleMenu() {
    ...
    this.#battleTextGameObjectLine2 = this.#scene.add.text(
      20,
      512,
      `${this.#activePlayerIngredient.name} do next?`,
      BATTLE_UI_TEXT_STYLE
    );
  ...
  #createIngredientAttackSubMenu() {
    ...
    const attackNames = [];
    for (let i = 0; i < 4; i += 1) {
      attackNames.push(this.#activePlayerIngredient.attacks[i]?.name || '-');
    }
    ...
    this.hideIngredientAttackSubMenu();
  }
  ...
  #handlePlayerChooseMainBattleOption() {
    ...
    if (this.#selectedBattleMenuOption === BATTLE_MENU_OPTIONS.FIGHT) {
      this.showIngredientAttackSubMenu();
      return;
    }
    ...
    if (this.#selectedBattleMenuOption === BATTLE_MENU_OPTIONS.SWITCH) {
      ...
      this.#switchIngredientAttempt = true;
      return;
    }
  ...
  #handleSceneResume(sys, data) {
    ...
    if (data && data.wasIngredientSelected) {
      // do nothing since new active Ingredient was chosen to switch to
      return;
    }
